...
class IotDevice(models.Model):
    ...
    def parse_multi_input(self, device_identification, passphrase, values):
        ...
        if not device:
            _logger.warning(
                "Device with identification %s not found", device_identification
            )
            return {"status": "error", "message": _("Server Error. Check server logs")}
        ...
        if not values:
            ...
            return {"status": "error", "message": _("Empty values array")}
        res = []
        for d in values:
